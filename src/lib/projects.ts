
export const projects = [
  {
    title: 'Personal Portfolio Website',
    achievements: [
      'Showcased skills and projects in a visually appealing and interactive Next.js application.',
      'Implemented a fully responsive design with Tailwind CSS for optimal viewing on all devices.',
      'Integrated a dynamic project filtering system to enhance user navigation.',
    ],
    live: 'https://github.com/Princekrcoder',
    category: 'web-development',
    featured: false,
  },
  {
    title: 'DevJobs - Job Portal Platform',
    achievements: [
      'Built a MERN stack platform for job seekers and employers, featuring job postings, applications, and user profiles.',
      'Developed advanced search and filtering capabilities for a seamless job search experience with Next.js.',
      'Created a clean and responsive user interface using Tailwind CSS for both applicants and recruiters.',
    ],
    live: 'https://github.com/Princekrcoder',
    category: 'web-development',
    featured: false,
  },
  {
    title: 'InsightInk - Modern Blog Platform',
    achievements: [
      'Developed a full-stack blogging application with Next.js, allowing users to create, edit, and publish articles.',
      'Engineered a secure backend with Node.js and MongoDB for user authentication and content management.',
      'Designed a minimalist and responsive reading experience with Tailwind CSS, focusing on content clarity.',
    ],
    live: 'https://github.com/Princekrcoder',
    category: 'web-development',
    featured: false,
  },
  {
    title: 'ConnectSphere - Real-time Chat Application',
    achievements: [
      'Created a real-time chat application using the MERN stack and Socket.IO for instant messaging.',
      'Implemented features like one-on-one and group chats, online status indicators, and user authentication.',
      'Built a responsive and intuitive chat interface with Next.js and Tailwind CSS for a seamless user experience.',
    ],
    live: 'https://github.com/Princekrcoder',
    category: 'web-development',
    featured: false,
  },
  {
    title: 'VibeVerse - Social Media Platform',
    achievements: [
      'Developed a MERN stack social media platform with features like posts, comments, likes, and follows.',
      'Utilized Next.js for a fast, SEO-friendly frontend with dynamic content loading and user profiles.',
      'Designed an engaging and responsive UI with Tailwind CSS, creating a modern social networking experience.',
    ],
    live: 'https://github.com/Princekrcoder',
    category: 'web-development',
    featured: false,
  },
  {
    title: 'LearnSphere - Online Learning Management System',
    achievements: [
      'Developed a full-featured LMS with Next.js, enabling course creation, student enrollment, and progress tracking.',
      'Built a secure MERN stack backend with user authentication and role-based access control (students, instructors).',
      'Designed a responsive and intuitive UI with Tailwind CSS for a seamless learning experience across devices.',
    ],
    live: 'https://github.com/Princekrcoder',
    category: 'web-development',
    featured: true,
  },
  {
    title: 'NexusAdmin - Comprehensive Admin Dashboard',
    achievements: [
        'Architected a powerful admin panel for data visualization and content management using Next.js.',
        'Implemented secure RESTful APIs on a Node.js/Express backend to manage users, products, and site analytics.',
        'Utilized Tailwind CSS to create a clean, data-driven interface with customizable widgets and reports.',
    ],
    live: 'https://github.com/Princekrcoder',
    category: 'web-development',
    featured: true,
  },
    {
    title: 'TaskFlow - Collaborative Project Management Tool',
    achievements: [
      'Created a Kanban-style project management tool with drag-and-drop functionality using React and Next.js.',
      'Developed real-time collaboration features with a MERN stack backend for assignments and comments.',
      'Ensured a responsive and user-friendly design with Tailwind CSS to improve team productivity.',
    ],
    live: 'https://github.com/Princekrcoder',
    category: 'web-development',
    featured: true,
  },
  {
    title: 'ShopWise - Modern E-commerce Platform',
    achievements: [
      'Built a complete MERN stack e-commerce site with product catalogs, shopping cart, and secure checkout.',
      'Leveraged Next.js for server-side rendering (SSR) to enhance SEO and initial page load performance.',
      'Styled the entire application with Tailwind CSS for a pixel-perfect and fully responsive online store.',
    ],
    live: 'https://github.com/Princekrcoder',
    category: 'web-development',
    featured: true,
  },
  {
    title: 'RealtyConnect - Real Estate Marketplace',
    achievements: [
        'Developed a Next.js real estate app with advanced property search, filtering, and map integration.',
        'Engineered a robust MERN stack backend to manage property listings, user profiles, and inquiries.',
        'Designed an elegant and responsive interface with Tailwind CSS for an optimal property viewing experience.',
    ],
    live: 'https://github.com/Princekrcoder',
    category: 'web-development',
    featured: true,
  },
];
